    ...
    embeddedServer(Netty, host = "0.0.0.0", port = port) {
        ...
        routing {
            ...
            post("/sync/media/{fileName}") {
                ...
                val mediaDir = java.io.File(System.getProperty("user.home"), ".ember/media").apply { mkdirs() }
                val file = java.io.File(mediaDir, fileName)
            ...
            get("/sync/media/{fileName}/upload-status") {
                ...
                call.respond(com.ben.ember.domain.sync.MediaUploadStatus(receivedBytes))
            }
            ...
            get("/sync/media/list") {
                ...
                val entries = (mediaDir.listFiles() ?: emptyArray())
                    .filter { it.isFile }
                    .map { com.ben.ember.domain.sync.RemoteMediaEntry(fileName = it.name, lastModified = it.lastModified()) }
                call.respond(com.ben.ember.domain.sync.RemoteMediaList(entries))
            }
